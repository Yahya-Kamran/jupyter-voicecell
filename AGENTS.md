...
```bash
python -m py_compile voicecell/__init__.py  # Check single file for syntax errors
```
...
```typescript
const PLUGIN_ID = 'voicecell:plugin';
```
...
export namespace CommandIDs {
  export const openPanel = 'voicecell:open-panel';
  export const refreshData = 'voicecell:refresh-data';
}
...
- **Output**: Creates files in `lib/` and `voicecell/labextension/`
...
- Ask user to search for the extension ID (`voicecell`) to see if it loaded
...
ls -la voicecell/labextension/  # Should contain bundled extension
```
...
- **Plugin ID** (in `src/index.ts`): `'voicecell:plugin'`
- **Command IDs** (in `src/commands.ts` or `src/index.ts`): `'voicecell:command-name'`
...
- **CSS classes** (in `style/index.css`): `.jp-voicecell-ClassName`
